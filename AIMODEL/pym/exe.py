...
# Load the dataset
df = pd.read_csv('D:\\Data_Desc_Season.csv')

# Strip whitespace from column names
df.columns = df.columns.str.strip()

# Print the columns and the first few rows to debug
...
# Print the combined features to debug
...
def index():
    ...
    if request.method == 'POST':
        ...
        user_activity = request.form['activity']
        ...
        # Get the indices of the top 5 most similar tourist spots
        similar_indices = cosine_similarities.argsort()[-5:][::-1]
        
        # Get the recommended spots based on the indices
        recommended_spots = df.iloc[similar_indices][['CITY', 'STATE', 'TOURIST SPOT', 'TYPE OF  ATTRACTION']]
    
    return render_template('index.html', recommended_spots=recommended_spots)
