...
def index():
    ...
    if request.method == 'POST':
        ...
        if not user_season or not user_activity:
            ...
            # Get indices of all relevant tourist spots (cosine similarity > 0)
            relevant_indices = [i for i, score in enumerate(cosine_similarities) if score > 0]
            
            # Get the recommended spots based on the indices
            recommended_df = df.iloc[relevant_indices][['CITY', 'STATE', 'TOURIST SPOT', 'TYPE OF ATTRACTION']]
